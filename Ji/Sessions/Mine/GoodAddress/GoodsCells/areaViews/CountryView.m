...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *countryString = model.name;
    if (![countryString isKindOfClass:[NSNull class]]) {
        cell.nameLabel.text  = countryString;
    }
    return cell;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [DBMineService regionWithParentId: integerToString(model.id) block:^(BOOL success, id result) {
        if (success) {
            ...
            self.provinView = [[ProvinView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH, 0, self.table.frame.size.width, self.frame.size.height)andArray:areaList];
            [self addSubview:self.provinView];

            //传下去
            self.provinView.countryId = integerToString(model.id);
            self.provinView.countryName = model.name;
            
            //出现
            [UIView animateWithDuration:0.5 animations:^{
                self.provinView.frame = CGRectMake(0, 0, self.table.frame.size.width, self.frame.size.height);
            } completion:^(BOOL finished) {
            }];
